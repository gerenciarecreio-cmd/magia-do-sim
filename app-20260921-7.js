...
async function hydrateSession(session){
  ...
  if(state.role==='admin'){
    ...
    if(wedding){
      await loadWeddingData(wedding.id);
      await loadCouplePhoto();
    } else {
...
function shellView(r,content){
  ...
  const clientClass=state.role==='client'?'client-shell':'admin-shell';
  const avatar=state.role==='client' && state.couplePhotoUrl
    ? `<div class="avatar avatar-photo"><img src="${esc(state.couplePhotoUrl)}" alt="Foto do casal"></div>`
    : `<div class="avatar">${first}</div>`;
  return `<div class="app-shell ${clientClass}">
    <aside class="sidebar">
    ...
    <main class="main">
      <header class="topbar">
        ...
        <div class="topbar-right"><button class="icon-btn" id="bell" aria-label="Notificações">${icons.bell}</button><a class="profile-chip" href="#/perfil">${avatar}<span class="small">${esc(displayName)}⌄</span></a></div>
      </header>
      ${content}
    </main>
...
function dashboardView(){
  ...
  return `<div class="page client-dashboard">
    <section class="client-hero card">
      <div class="client-hero-copy">
        <span class="client-hero-kicker">A MAGIA DO SIM</span>
        <h1 class="hero-title">Olá, ${esc(partnerNames())}! ♡</h1>
        <p class="hero-sub">Que essa jornada seja tão especial quanto o grande dia.</p>
        <div class="date">${icons.calendar} ${esc(dateLong(state.wedding.wedding_date))}</div>
      </div>
      <div class="client-hero-countdown">
        ${c.text
          ? `<span class="countdown-label">Grande dia</span><strong class="countdown-number compact">${esc(c.text)}</strong>`
          : `<span class="countdown-label">Faltam</span><strong class="countdown-number">${c.days}</strong><span class="countdown-unit">dias para o grande dia ♡</span>`}
      </div>
    </section>

    <section class="dashboard-main">
      <div class="card card-pad planning-card"><div class="card-title"><h2>Seu planejamento</h2><span class="sub">${comp}% concluído</span></div><div class="progress-track"><div class="progress-fill" style="width:${comp}%"></div></div><div class="steps">${[['Local',!!state.wedding.venue],['Fotografia',state.vendors.some(v=>v.category==='Fotografia'&&v.status==='Contratado')],['Buffet',state.vendors.some(v=>v.category==='Buffet'&&v.status==='Contratado')],['Decoração',state.vendors.some(v=>v.category==='Decoração'&&v.status!=='Pendente')],['Música',state.vendors.some(v=>v.category==='Música'&&v.status==='Contratado')]].map(([t,done])=>`<div class="step ${done?'done':''}"><div class="step-dot">${done?'✓':'○'}</div>${t}</div>`).join('')}</div></div>
...
function weddingView(){
  ...
  const photo=state.couplePhotoUrl
    ? `<div class="couple-photo-card"><img src="${esc(state.couplePhotoUrl)}" alt="Foto do casal"></div>`
    : `<a href="#/perfil" class="couple-photo-card couple-photo-empty"><span>♡</span><small>Adicionar foto do casal</small></a>`;
  ...
  <div class="card detail-hero client-wedding-hero">${photo}<div class="client-wedding-info"><p class="small muted">CASAMENTO</p><h2 class="couple-name">${esc(w.couple_name)}</h2><div class="detail-grid">
...
function docsView(){
  ...
  const action=state.role==='admin'?'<button class="btn-primary" id="upload-doc">+ Enviar documento</button>':'';
  return `<div class="page"><div class="page-head"><div><h1>Documentos</h1><p>Acesse contratos, orçamentos e arquivos importantes do casamento.</p></div>${action}</div>
  ...
  <div class="card list-card">${items.length?items.map(d=>`<div class="list-row doc-row"><div class="doc-icon">${icons.file}</div><div class="vendor-name"><strong>${esc(d.name)}</strong><span>Arquivo do casamento</span></div><div class="doc-type small muted">${esc(d.type)}</div><div class="doc-date small muted">${esc(d.date)}</div><button class="btn-secondary" data-view-doc="${d.id}">Visualizar</button></div>`).join(''):emptyState('Ainda não há documentos.','Os documentos enviados pela assessoria aparecerão aqui.')}</div></div>`;
}
...
function bindView(r){
  ...
  document.querySelectorAll('[data-view-doc]').forEach(b=>b.onclick=async()=>{const d=state.docs.find(x=>x.id===b.dataset.viewDoc); if(!d?.path){toast('Este documento ainda não possui arquivo vinculado.');return;} if(/^https?:\/\//i.test(d.path)){window.open(d.path,'_blank','noopener');return;} const {data,error}=await sb.storage.from('wedding-documents').createSignedUrl(d.path,120); if(error||!data?.signedUrl){console.error(error);toast('Não foi possível abrir o documento.');return;} window.open(data.signedUrl,'_blank','noopener');});
  ...
  const cp=document.getElementById('upload-couple-photo'); if(cp) cp.onclick=uploadCouplePhoto;
}
...
function openDocumentEditor(){
  if(state.role!=='admin'){toast('Somente a assessoria pode enviar documentos.');return;}
  const body=field('Nome do documento','name','')+selectField('Tipo','document_type',['Contrato','Orçamento','Outro'],'Outro')+`<div class="field"><label>Arquivo</label><input class="input" type="file" name="file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" required><small class="muted">PDF, Word ou imagem.</small></div>`;
  modal('Enviar documento',body,'Enviar',async back=>{
    ...
    const path=`${state.wedding.id}/${Date.now()}-${safeName}`;
    ...
    if(uploadError){console.error(uploadError);toast('Não foi possível enviar o arquivo.');if(submit){submit.disabled=false;submit.textContent='Enviar';}return false;}
    const {error:dbError}=await sb.from('documents').insert({wedding_id:state.wedding.id,name,document_type:f.document_type,file_path:path});
    if(dbError){console.error(dbError);await sb.storage.from('wedding-documents').remove([path]);toast('Não foi possível cadastrar o documento.');if(submit){submit.disabled=false;submit.textContent='Enviar';}return false;}
    await loadWeddingData(state.wedding.id);
...
async function loadCouplePhoto(){
  ...
  if(!path) return;
  const {data,error}=await sb.storage.from('couple-profile-photos').createSignedUrl(path,86400);
  if(error){console.error(error);return;}
  state.couplePhotoUrl=data?.signedUrl||'';
}

async function uploadCouplePhoto(){
  if(state.role!=='client'||!state.wedding){toast('Casamento não encontrado.');return;}
  const input=document.getElementById('couple-photo-file');
  ...
  const btn=document.getElementById('upload-couple-photo');
  ...
  try{
    ...
    if(rpcError){
      ...
      toast('Não foi possível salvar a foto no perfil.');
      return;
    }
    ...
    if(btn){btn.disabled=false;btn.textContent='Atualizar foto do casal';}
  }
}
...
sb.auth.onAuthStateChange((event,session)=>{
  if(event==='SIGNED_OUT'){state.session=null;state.user=null;state.profile=null;state.wedding=null;state.couplePhotoUrl='';state.loading=false;render();}
